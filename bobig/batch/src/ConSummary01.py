...
def Summary_file(ori_values_column,var_cd_col,meta_val,prvdr_cd, cat_cd,RSLT_PTH_NM,
    ...
    try:
        ...
        #CNT컬럼을 추가
        ori_values_column.extend(['CNT'])
        #원본파일에 CNT 추가

        ori_file['CNT']= 0

        msg = "요약파일 컬럼 => {} ".format(ori_values_column)
        ...
        ori_sumjob_file = ori_file[ori_values_column].set_index('ASK_ID')

        # 리스트에 CNT 컬럼 삭제 (group 시 사용는 CNT 값이 들어가면 안되서 리스트에서 삭제 함)
        ori_values_column.remove('CNT')

        #CNT를 제외한 컬럼들을 groupby로 묶음
        SumJob_Summary_groupby = ori_sumjob_file.groupby(ori_values_column)
        ...
        try:
            os.mkdir(bobig_sumjob01_dir)
        except:
            pass
        #비식별 요약 저장경로 생성
        try:
            os.mkdir(bobig_sumjob02_dir)
        except:
            pass
        ...
        SumJob_Summary_groupby = SumJob_subset.groupby(ori_values_column)
            ...
            if len(TBPINM102_meta_sel_01_fetchall) != 0:
                ...
                for meta_val in TBPINB103_SEL_02_fetchall:
                    ...
                    if os.path.isfile(BOBIG_PROV_IN_DIR + '/' + file_nm):
                        #쓰레드 summary함수
                        t = threading.Thread(target=Summary_file,
                                             args=[sumjob_col, var_cd_col,meta_val, PRVDR_CD, CAT_CD, RSLT_PTH_NM,
                                                   chr_col_num,chr_col_name,int_col_num,int_col_name,CNT_COL])
                        t.start()
                        threads.append(t)
