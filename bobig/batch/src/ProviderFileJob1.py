...
def main():
    ...
    TBPGPM101_sel_01_src = SQL_DIR + '/' + 'T_BDREQUESTINFO_sel_01.sql'
    TBPGPM101_sel_01 = query_seperator(TBPGPM101_sel_01_src).format(ask_id = ASK_ID)
    logging.info(TBPGPM101_sel_01)
    print(TBPGPM101_sel_01)
    cur.execute(TBPGPM101_sel_01)
    TBPGPM101_sel_01_fetchall = cur.fetchall()

    if len(TBPGPM101_sel_01_fetchall) == 0 :
        logging.info('신청번호가 {} 인 대상이 없습니다.'.format(ASK_ID))
    ...
    for meta_val in TBPGPM101_sel_01_fetchall:
        ...
        try:
            ...
            target_file_sel_01_src = SQL_DIR + '/' + 'TBPINB103_SEL_01.sql'
            target_file_sel_01 = query_seperator(target_file_sel_01_src).format(ask_id = ask_id,
                                                                                rshp_id =rshp_id)
            logging.info(target_file_sel_01)
            print(target_file_sel_01)
            ...
            for target_file_val in target_file_sel_01_fetchall:
                ...
                PROV_IN_DIR = Bobig_ori_prov_path()   # '/esb_nfs/2019/20210201_HN'      
                # PROV_IN_DIR = '/esb_nfs/2019/20210201_HN'
                if platform.system() == 'Windows' and PRVDR_CD == 'K0004':
                    PROV_IN_DIR = 'D:/work/2019/out'                
                print('{}/{} 확인'.format(PROV_IN_DIR, FILE_NM))
                ...
                if os.path.isfile(PROV_IN_DIR + '/' + FILE_NM):
                    ...
                    if File_Naming_Check(ask_id, rshp_id, PRVDR_CD, CAT_CD, FILE_NM):
                            ...
                            row = [PRVDR_CD,CAT_CD,FILE_NM]
                            FILE_DF = FILE_DF.append(pd.Series(row, index=FILE_DF.columns), ignore_index=True)
                            logging.info('{} 파일이름 OK'.format(FILE_NM))
                            print('{} 파일이름 OK'.format(FILE_NM))
                    else:
                    ...
                    err_msg = '파일 없음 : {file_nm}'.format(FILE_NM)
                    # TBPGPB103_upt('N', err_msg, ask_id, rshp_id, FILE_NM)
                    #TBPGPV301_ins(OID,ASK_ID,RSHP_ID,'4297','실패',err_msg)
                    logging.warning(err_msg)
                    print(err_msg)
            ...
            for idx in FILE_DF.index:
                ...
                #PROV_IN_DIR = Agency_prov_in_path(PRVDR_CD)
                PROV_IN_DIR = Bobig_ori_prov_path()          
                if platform.system() == 'Windows' and PRVDR_CD == 'K0004':
                    PROV_IN_DIR = 'D:/work/2019/out'  
                ...
                msg = '{} 파일이름 확인 완료. 정합성검증 시작'.format(FILE_NM)
                logging.info(msg)
                print(msg)
                ...
                #                       on=['ASK_ID', 'RSHP_ID', 'PRVDR_CD', 'MRG_NM', 'MRG_SSN'])
                # 메타정보 (ask_id, rshp_id, prvdr_cd,hash_did)
                # data_meta = data_csv1[['ASK_ID', 'RSHP_ID', 'PRVDR_CD','HASH_DID']]
                #tot_data = pd.concat([data_meta,data_val], axis=1)#.set_index(['ASK_ID'])
                #data_csv1 = tot_data
                ...
                if not Con_Check:
                    err_msg = '{} 정합성 검증 실패'.format(ask_id)
