                                ...
                                if deid_val[1] is None:
                                    ...
                                    # 변환할 타입이 string이 아닌경우 (int64 or float 등)
                                        ...
                                        if change_type == 'Int64':
                                            temp = pd.DataFrame((pd.to_numeric(ClassicDeId_data[col_nm], errors='coerce').astype(np.int64)).fillna(-999))
                                        else : 
                                            temp = pd.DataFrame((pd.to_numeric(ClassicDeId_data[col_nm], errors='coerce').astype(np.float)).fillna(-999))
